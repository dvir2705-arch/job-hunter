...
def make_job(title: str, company: str = "Acme") -> JobListing:
    return JobListing(
        title=title, company=company, location="Israel", url="http://example.com"
    )
...
def test_accept_machine_learning_researcher(monkeypatch):
    ...
    profile = UserProfile(
        ...
        phone="0",
        skills=[],
        target_positions=["machine learning"],
        domains=[],
    )
    monkeypatch.setattr("job_hunter.jobs.relevance_filter.get_profile", lambda: profile)
...
def test_accept_embedded_software_engineer(monkeypatch):
    ...
    profile = UserProfile(
        ...
        phone="0",
        skills=[],
        target_positions=["embedded", "software"],
        domains=["embedded", "software"],
    )
    monkeypatch.setattr("job_hunter.jobs.relevance_filter.get_profile", lambda: profile)
...
def test_accept_dsp_algorithm_engineer(monkeypatch):
    ...
    profile = UserProfile(
        ...
        phone="0",
        skills=[],
        target_positions=["DSP"],
        domains=["dsp", "algorithm"],
    )
    monkeypatch.setattr("job_hunter.jobs.relevance_filter.get_profile", lambda: profile)
...
def test_irrelevant_and_relevant_goes_uncertain(monkeypatch):
    ...
    with patch(
        "job_hunter.jobs.relevance_filter._batch_haiku_filter",
        return_value=([make_job("Software Quality Assurance Engineer")], []),
    ):
...
def test_uncertain_job_accepted_by_batch_haiku():
    ...
    with patch(
        "job_hunter.jobs.relevance_filter._haiku_batch_call",
...
def test_uncertain_multiple_jobs_batch():
    ...
    jobs = [
        make_job("Operations Intern"),
        make_job("General Student"),
        make_job("Project Intern"),
    ]
    with patch(
        "job_hunter.jobs.relevance_filter._haiku_batch_call",
...
class TestEmptyDomains:
    ...
    def _mock_profile(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "job_hunter.jobs.relevance_filter.get_profile",
            lambda: profile,
        )
