...
class TestNormalizeCvSchema:
    ...
    def test_experience_existing_highlights_untouched(self):
        cv = {"experience": [{"position": "Dev", "highlights": ["Built API", "Led team"]}]}
        result = normalize_cv_schema(cv)
        assert result["experience"][0]["highlights"] == ["Built API", "Led team"]

    def test_education_build_year_from_dates(self):
        cv = {"education": [{"institution": "BGU", "start_date": "2023", "end_date": "Present"}]}
        result = normalize_cv_schema(cv)
    ...
    def test_education_existing_year_preserved(self):
        cv = {"education": [{"institution": "BGU", "degree": "BSc", "year": "2023-Present"}]}
        result = normalize_cv_schema(cv)
        assert result["education"][0]["year"] == "2023-Present"

    def test_military_title_to_role_and_period(self):
        cv = {"military": [{"title": "Logistics NCO", "company": "IDF", "start_date": "2017", "end_date": "2020"}]}
        result = normalize_cv_schema(cv)
    ...
    def test_volunteering_aliases(self):
        cv = {"volunteering": [{"name": "Scouts", "title": "Leader", "start_date": "2015", "end_date": "2017"}]}
        result = normalize_cv_schema(cv)
    ...
    def test_idempotent_on_canonical_schema(self):
        ...
        cv = {
            ...
            "experience": [
                {"company": "Acme", "position": "Engineer", "start": "2022", "end": "Present",
                 "highlights": ["Led migration"]}
            ],
            "education": [
                {"institution": "BGU", "degree": "BSc EE", "year": "2023-Present", "gpa": "82",
                 "highlights": ["Linear Algebra: 100"]}
            ],
            "military": [
                {"unit": "401st", "role": "Combat", "period": "2019-2022",
    ...
    def test_ben_full_experience_normalized(self):
        ...
        assert exp["position"] == "\u05de\u05e4\u05e2\u05d9\u05dc \u05d7\u05e0\u05d5\u05ea"
        assert exp["company"] == "HOKA"
        assert exp["start"] == "\u05d0\u05d5\u05d2\u05d5\u05e1\u05d8 2025"
        assert exp["end"] == "\u05d4\u05d9\u05d5\u05dd"
        assert exp["highlights"] == ["\u05de\u05db\u05d9\u05e8\u05d5\u05ea \u05e9\u05d8\u05d7"]
...
class TestTemplateRendering:
    ...
    def test_english_rendering_ltr(self):
        ...
        cv = {
            ...
            "summary": "A developer.", "experience": [], "education": [],
            "skills": ["Python"],
        }
    ...
    def test_hebrew_rendering_rtl(self):
        ...
        cv = {
            "name": "\u05d1\u05df", "title": "\u05de\u05e4\u05ea\u05d7",
            "contact": {"email": "test@test.com", "phone": "123"},
            "summary": "\u05ea\u05e7\u05e6\u05d9\u05e8", "experience": [], "education": [],
            "skills": ["Python"],
        }
        html = renderer.render_html(cv, lang="he")
        assert 'lang="he"' in html
        assert 'dir="rtl"' in html
        assert "\u05ea\u05e7\u05e6\u05d9\u05e8" in html  # Hebrew section title for Summary
    ...
    def test_hebrew_section_titles(self):
        ...
        cv = {
            ...
            "summary": "x", "experience": [
                {"position": "Dev", "company": "X", "start": "2023", "end": "Now",
                 "highlights": ["Did stuff"]}
            ],
            "education": [{"institution": "U", "degree": "BSc", "year": "2023"}],
            "skills": ["Python"],
            "military": [{"unit": "IDF", "role": "Soldier", "period": "2020",
                         "highlights": ["Served"]}],
        }
        html = renderer.render_html(cv, lang="he")
        assert "\u05e0\u05d9\u05e1\u05d9\u05d5\u05df \u05ea\u05e2\u05e1\u05d5\u05e7\u05ea\u05d9" in html  # ניסיון תעסוקתי
        assert "\u05d4\u05e9\u05db\u05dc\u05d4" in html  # השכלה
        assert "\u05db\u05d9\u05e9\u05d5\u05e8\u05d9\u05dd" in html  # כישורים
        assert "\u05e9\u05d9\u05e8\u05d5\u05ea \u05e6\u05d1\u05d0\u05d9" in html  # שירות צבאי
